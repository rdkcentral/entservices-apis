...
The power mode change flow includes two acknowledgement phases:
1. Pre-change notification phase - clients can delay the change using `DelayPowerModeChangeBy`
2. Acknowledgement phase - clients must acknowledge readiness before the actual power state change occurs

If no acknowledgement clients are registered, the acknowledgement phase is automatically skipped.
...
| Method | Description |
| :-------- | :-------- |
| [addPowerModeChangeAcknowledgementClient](#addPowerModeChangeAcknowledgementClient) | Registers a client for power mode change acknowledgement notifications. Returns an `acknowledgeClientId` that the client must use when calling `PowerModeChangeAcknowledgement`. |
...
| [powerModeChangeAcknowledgement](#powerModeChangeAcknowledgement) | Called by clients to acknowledge a power mode change request during the acknowledgement phase. Clients receive the `transactionId` via the `OnPowerModeChangeAcknowledgementRequested` notification and must call this method to confirm they are ready for the power state change to proceed. |
...
| [removePowerModeChangeAcknowledgementClient](#removePowerModeChangeAcknowledgementClient) | Unregisters a client from power mode change acknowledgement notifications. If called during an active acknowledgement round, the client automatically acknowledges. |
...
Registers a client for power mode change acknowledgement notifications. Returns an `acknowledgeClientId` that the client must use when calling `PowerModeChangeAcknowledgement`.
...
| :-------- | :-------- | :-------- |
| result | object |  |
| result.acknowledgeClientId | integer | Unique identifier for the client to be used when calling `PowerModeChangeAcknowledgement` |
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 2, "method": "org.rdk.PowerManager.delayPowerModeChangeBy", "params": {"clientId": 0, "transactionId": 0, "delayPeriod": 0}}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 3, "method": "org.rdk.PowerManager.getNetworkStandbyMode"}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 4, "method": "org.rdk.PowerManager.getPowerState"}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 5, "method": "org.rdk.PowerManager.getPowerStateBeforeReboot"}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 6, "method": "org.rdk.PowerManager.getTemperatureThresholds"}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 7, "method": "org.rdk.PowerManager.getThermalState"}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 8, "method": "org.rdk.PowerManager.getTimeSinceWakeup"}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 9, "method": "org.rdk.PowerManager.getWakeupSourceConfig"}' http://127.0.0.1:9998/jsonrpc
```
...
Called by clients to acknowledge a power mode change request during the acknowledgement phase. Clients receive the `transactionId` via the `OnPowerModeChangeAcknowledgementRequested` notification and must call this method to confirm they are ready for the power state change to proceed.
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 10, "method": "org.rdk.PowerManager.powerModeChangeAcknowledgement", "params": {"acknowledgeClientId": 0, "transactionId": 0}}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 11, "method": "org.rdk.PowerManager.powerModePreChangeComplete", "params": {"clientId": 0, "transactionId": 0}}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 12, "method": "org.rdk.PowerManager.reboot", "params": {"rebootRequestor": "", "rebootReasonCustom": "", "rebootReasonOther": ""}}' http://127.0.0.1:9998/jsonrpc
```
...
Unregisters a client from power mode change acknowledgement notifications. If called during an active acknowledgement round, the client automatically acknowledges.
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 13, "method": "org.rdk.PowerManager.removePowerModeChangeAcknowledgementClient", "params": {"acknowledgeClientId": 0}}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 14, "method": "org.rdk.PowerManager.removePowerModePreChangeClient", "params": {"clientId": 0}}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 15, "method": "org.rdk.PowerManager.setPowerState", "params": {"keyCode": 0, "powerState": "UNKNOWN", "reason": ""}}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 16, "method": "org.rdk.PowerManager.setTemperatureThresholds", "params": {"high": 0.0, "critical": 0.0}}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 17, "method": "org.rdk.PowerManager.setWakeupSourceConfig", "params": [{"wakeupSource": "UNKNOWN", "enabled": true}]}' http://127.0.0.1:9998/jsonrpc
```
...
| [onPowerModeChangeAcknowledgementRequested](#onPowerModeChangeAcknowledgementRequested) | Notified when a power mode change is about to occur and requires acknowledgement from registered clients. This is sent after the pre-change notification phase completes successfully. Clients must respond by calling `PowerModeChangeAcknowledgement` with the provided `transactionId` within the timeout period (default 10 seconds). |
...
| params.transactionId | int | transactionId to be used when invoking PowerModeChangeAcknowledgement() |
| params.reason | string | Reason for moving to the power state |
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 25, "method": "org.rdk.PowerManager.getLastWakeupKeyCode"}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 26, "method": "org.rdk.PowerManager.getLastWakeupReason"}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 27, "method": "org.rdk.PowerManager.getOvertempGraceInterval"}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 28, "method": "org.rdk.PowerManager.setDeepSleepTimer", "params": {"timeOut": 0}}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 29, "method": "org.rdk.PowerManager.setNetworkStandbyMode", "params": {"standbyMode": true}}' http://127.0.0.1:9998/jsonrpc
```
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 30, "method": "org.rdk.PowerManager.setOvertempGraceInterval", "params": {"graceInterval": 0}}' http://127.0.0.1:9998/jsonrpc
```
