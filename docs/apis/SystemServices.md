...
| params.wakeupSources[#].wakeupSource | int | Wakeup source type (must be one of the following: 0 - WAKEUP_SRC_UNKNOWN, 1 - WAKEUP_SRC_VOICE, 2 - WAKEUP_SRC_PRESENCEDETECTED, 3 - WAKEUP_SRC_BLUETOOTH, 4 - WAKEUP_SRC_WIFI, 5 - WAKEUP_SRC_IR, 6 - WAKEUP_SRC_POWERKEY, 7 - WAKEUP_SRC_TIMER, 8 - WAKEUP_SRC_CEC, 9 - WAKEUP_SRC_LAN, 10 - WAKEUP_SRC_RF4CE) |
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 44, "method": "org.rdk.SystemServices.setWakeupSrcConfiguration", "params": {"powerState": "", "wakeupSources": [{"wakeupSource": 1, "enabled": true}]}}' http://127.0.0.1:9998/jsonrpc
```
