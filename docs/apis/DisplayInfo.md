...
Initializes and caches the current display frame rate. Caches the current frame rate so subsequent changes can be detected and notified via the FRAMERATE_CHANGE event.
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 23, "method": "org.rdk.DisplayInfo.initializeFrameRate"}' http://127.0.0.1:9998/jsonrpc
```
