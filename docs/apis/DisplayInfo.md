...
| [initializeFrameRate](#initializeFrameRate) | Initializes the frame rate information |
...
Initializes and caches the current display frame rate.
...
```curl
curl -H 'content-type:text/plain;' --data-binary '{"jsonrpc": 2.0, "id": 3, "method": "org.rdk.DisplayInfo.initializeFrameRate"}' http://127.0.0.1:9998/jsonrpc
```
